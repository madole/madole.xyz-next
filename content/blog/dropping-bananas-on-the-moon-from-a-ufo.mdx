...
To do this, I will use the `viewer.clock.onTick` event to run the callback every time the clock ticks. I’ll then `lerp` between the start position and the end position based on the `currentTime - startTime` . I’ll then set the position to be the new position in the animation and request a rerender show it shows. When the animation is complete, I will remove the `onTick` event listener.
