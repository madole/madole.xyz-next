...
```javascript
const imageUrl = 'https://images.unsplash.com/photo-1601143240380-10dfad37660d?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=60';
const imageDirAndName = './downloaded-img.jpg';

fetch(imageUrl)
    .then(response => response.body.pipe(fs.createWriteStream(imageDirAndName)).on('close', () => console.log('image downloaded')))
