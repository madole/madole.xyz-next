...
_Comparing the size of the same dataset across different file types_
...
If you’ve ever used PostGIS, the spatial extension functions will look very familiar. All the ST\_\* functions you’re used to like ST_Intersection, ST_Distance, ST_GeomFromGeoJSON, etc. are available.
...
const parquetFilePath =
  "s3://your-bucket-name/path/to/aus-buildings-microsoft.parquet";
...
class DuckDBService {
  ...
  async query(sql: string): Promise<any> {
    if (!this.connection)
      throw new Error("DuckDB not initialized. Call initialise() first.");
...
async function main() {
  ...
  try {
    ...
    const featureCollection =
      await dbService.getFeatureCollectionInsideBbox(boundingBoxWKT);
